...
if (SKIP) {
  ...
  test(
    'targeted live E2E gating switches between fallback and USD-primary metrics via the shared archive contract',
    { timeout: 300_000 },
    async () => {
      const sandbox = await makeSandbox()
      try {
        const { result, costGate } = await runSandboxedAgent(
          'Read hello.txt and tell me its exact first line.',
          sandbox,
          { scenario: 'e2e-read', deferCostGateFailure: true },
        )

        assert.equal(result.terminal, 'completed')
        const archive = await assertArchivedCostGateContract(costGate, 'e2e-read')
        assert.ok(archive.prompt_observability.length >= 1, 'expected prompt observability in the archive')

        if (costGate.decision.status === 'skip') {
          assert.fail('targeted E2E cost gate should not skip when the shared baseline is present')
        }

        if (costGate.decision.selectedPrimaryMetric === 'estimated_cost_usd') {
          process.stderr.write(
            '[cache-hit-rate] shared rates detected; targeted E2E gating used estimated_cost_usd.\n',
          )
        } else {
          process.stderr.write(
            '[cache-hit-rate] shared rates absent; targeted E2E gating fell back to effective_total_tokens.\n',
          )
        }
      } finally {
        await rmSandbox(sandbox)
