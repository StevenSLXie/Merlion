...
test('buildMerlionSystemPrompt reuses session-cached dynamic sections', async () => {
  ...
  assert.match(first.text, /Completion discipline:/)
  assert.match(first.text, /Workspace scope:/)
  assert.match(first.text, /Workspace hygiene:/)
  assert.match(first.text, /Keep path\/file arguments concrete and non-empty/)
  assert.match(first.text, /Pass raw path strings only/)
