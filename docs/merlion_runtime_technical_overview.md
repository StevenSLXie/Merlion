...
- tool registry 仍是“静态注册表”，没有 free-code 那种 tool pool assembly、mode filtering、MCP 合并层。
...
这是一个很克制的 contract，优点是简单，但也意味着还没有承载更复杂的 tool-side runtime 信息。

### 3.3 registry 的现状：简单但稳定
...
### 3.7 当前 tool registry 的短板

- 还没有 tool catalog / tool pool assembly。
- 没有 builtin / extension / MCP 三层来源模型。
- 没有 mode-based 过滤、deny-rule prefilter、stable sort assembly。
- tool metadata 不足以支撑 deferred loading、MCP provenance、read-only/destructive 分类。
